...
> **Read this before acting on anything below — this is the `rework` branch, and the plan is [`docs/REWORK-PLAN.md`](docs/REWORK-PLAN.md).** Read it before writing code here. It names the three modules being rewritten (`derive.ts`, `store.ts`, `storage.ts`), the modules being **ported unchanged with their tests** because they encode failures that cost real days, the target structure, and the bottom-up build order. Work follows that order; if a change does not fit the plan, edit the plan first and say why. Building feature-first without it is precisely what produced the attempt this one replaces.
>
> **Everything below this notice describes the archived first attempt**, which built, ran and passed 178 tests. Treat it as a *record of a working design*, not as a description of the current tree — during the rework the two diverge, and the plan says which parts are deliberately gone. It is still the best available account of the domain, and the **[Domain model](#domain-model) and [invariants](#the-invariants) sections carry forward essentially intact**: the vocabulary was not the problem. **The [Next step](#next-step) list is frozen** and is not this branch's backlog.
>
> The archive itself lives on `main` and `deprecated` (both `2dbfc10`), kept whole, including the `kind` / colchón feature that `CHANGELOG.md` still calls `Unreleased` — which is accurate, it was never deployed. Nothing there is deleted, so any part of it can be read or copied back.
